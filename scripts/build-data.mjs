...
const videos = availableRawVideos.map((video) => {
  ...
  return {
    ...
    titleHasBut: video.title_has_but === "yes" || /\bbut\b/i.test(video.title || ""),
...
const summary = {
  ...
  butTitleVideos: videos.filter((video) => video.titleHasBut).length,
  badAppleButVideos: videos.filter((video) => video.category === "bad_apple_but").length,
  otherVideos: videos.filter((video) => video.category === "other").length,
